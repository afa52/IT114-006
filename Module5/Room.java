...
public class Room {
	...
	//can use dictionary for non duplicate clients
	protected synchronized void addClient(ServerThread client) {
		...
		if (clients.indexOf(client) > -1) {
			...
			new Thread() {
				...
				//thread happens bc the clients room doesnt get set immeaditely so thats why we have to delay to enforce order
			}.start();
	...
	private boolean processCommands(String message, ServerThread client) {
		...
		try {
			if (message.startsWith("/")) {
				...
				String roomName;
				//string manipulation, split is one of the easy ones, when we split a string
				// /createroom test
				// [null, 'createroom']
				// "createroom test"
				// ['createroom', 'test']
				...
				//other ways
				// message.replace("/createroom","")
				//if we replace createroom with nothing, then all thats left is the name of the room
				switch (command) {
					case "createroom":
						roomName = comm2[1];
						if (server.createNewRoom(roomName)) { //if new room is created
							server.joinRoom(roomName, client); //have user join room
						} else {
							client.send(String.format("Room %s already exists", roomName)); //room already exists
						}
						wasCommand = true;
						break;
					case "joinroom":
